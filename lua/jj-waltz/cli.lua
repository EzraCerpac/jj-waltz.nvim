...
function M.friendly_error(result, context)
  ...
  if code == 127 or stderr:find("command not found", 1, true) or stderr:find("not found", 1, true) then
    return {
  ...
  if stderr:find("unrecognized subcommand 'links'", 1, true)
      or stderr:find("unexpected argument '--print-path'", 1, true)
      or stderr:find("unexpected argument '--no-links'", 1, true) then
    return {
      kind = "unsupported",
  ...
  if stderr:find("could not determine current workspace", 1, true)
      or stderr:find("failed to execute `jj", 1, true)
      or stderr:find("workspace root", 1, true)
      or stderr:find("There is no jj repo", 1, true) then
    return {
...
function M.ensure_capabilities(required)
  ...
  if required.print_path and not capabilities.print_path then
    ...
    if not capabilities.print_path then
      return nil, probe_err or M.friendly_error({
        code = 1,
